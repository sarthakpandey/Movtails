...
public class MovieCustomAdapterTypeTwo extends RecyclerView.Adapter<MovieCustomAdapterTypeTwo.ViewHolderPro> {

    private Context mContext;
    private ArrayList<MovieResponseGeneral> mMovies;

    public MovieCustomAdapterTypeTwo(Context mContext, ArrayList<MovieResponseGeneral> mMovies) {
    ...
    public ViewHolderPro onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ViewHolderPro(LayoutInflater.from(mContext).inflate(R.layout.adapter_two_layout,parent,false));
    }
    ...
    public void onBindViewHolder(final ViewHolderPro holder, int position) {
        ...
        Glide.with(mContext.getApplicationContext()).load(Constants.IMAGE_LOADING_BASE_URL_780 + mMovies.get(position).getBackdrop_path())
                .asBitmap()
                .centerCrop()
                .animate(animation)
